...
class EtcdService
{
    ...
    {
        return $this->get($this->prefix . "\0", ['range_end' => "\0"]);
    }
    ...
    public function grantRolePermission($role, $permType, $key, $rangeEnd = null)
    {
        $params = [
            ...
            'perm' => [
                'permType' => $permType,
                'key' => base64_encode($this->prefix . $key),
            ],
        ];
        if ($rangeEnd !== null) {
    ...
    protected function convertFields(array $data)
    {
        ...
        foreach ($data as $index => $value) {
            ...
            try {
                $value['value'] = Json::decode($value['value'], Json::TYPE_ARRAY);
